...
1.  **Nginx Instalado:** Utilize o [guia de instalação do Nginx a partir do repositório oficial](./INSTALACAO_NGINX.md).
2.  **Domínio Público:** Um nome de domínio registrado (ex: `afsimtech.com`) e um subdomínio (`wiki.afsimtech.com`) apontando (via registro DNS do tipo `A`) para o **IP público** do seu firewall/servidor.
3.  **Aplicação Interna:** Uma aplicação rodando em um IP privado na sua rede interna (ex: BookStack em `10.0.193.10`).
...
```bash
# Substitua 'wiki.afsimtech.com' pelo seu subdomínio
sudo nano /etc/nginx/conf.d/wiki.afsimtech.com.conf
```
...
```nginx
# Configuração para wiki.afsimtech.com

server {
    listen 80;
    server_name wiki.afsimtech.com;
    ...
    location / {
        ...
        proxy_pass [http://10.0.193.10](http://10.0.193.10); 
...
Neste ponto, ao acessar `http://wiki.afsimtech.com` no seu navegador, você já deve ver a página do seu BookStack.
...
4.  Listará os domínios encontrados (ex: `wiki.afsimtech.com`) e perguntará para qual deles você quer o certificado. Selecione o número correspondente e pressione Enter.

O Certbot obterá o certificado e **editará automaticamente** o seu arquivo (`/etc/nginx/conf.d/wiki.afsimtech.com.conf`) para configurar o SSL e redirecionar todo o tráfego HTTP para HTTPS.
...
Este método é ideal se seu servidor Nginx não está diretamente exposto na porta 80 ou se você quer um **certificado wildcard** (ex: `*.afsimtech.com`). Ele funciona criando um registro TXT temporário no seu DNS via API para provar o controle do domínio.
...
5.  Em **"Zone Resources"**, selecione `Include | Specific zone | seu.dominio` (ex: `afsimtech.com`).
...
sudo certbot certonly \
   ...
   -d '*.afsimtech.com' \
   --agree-tos \
   -m seu-email@afsimtech.com \
   --no-eff-email
```
O Certbot usará a API para criar os registros TXT, validar seu domínio e baixar os certificados para `/etc/letsencrypt/live/afsimtech.com/`.

### Passo 3.5: Atualizar a Configuração do Nginx Manualmente

Agora, edite novamente seu arquivo de configuração (`/etc/nginx/conf.d/wiki.afsimtech.com.conf`) para usar os certificados SSL. A configuração final deve se parecer com esta:
...
server {
    ...
    ssl_certificate /etc/letsencrypt/live/[afsimtech.com/fullchain.pem](https://afsimtech.com/fullchain.pem);
    ssl_certificate_key /etc/letsencrypt/live/[afsimtech.com/privkey.pem](https://afsimtech.com/privkey.pem);
